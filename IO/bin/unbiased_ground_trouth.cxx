...
void unbiased_ground_trouth::Run(std::string manual_labels_path, std::string store_image_path, int size_of_experts)
...
std::vector<string> folders_name =read_images_folder(mlp_expert,1,true);
	...
	*/
	...
	std:: vector<string> seg_im =read_images_folder(store_image_path+heart_phase+"/mask_manual"+"/"+ std::to_string(ex)+"/"+folders_name[k]+"/",1,false);
	patient_number.push_back(seg_im.size());

	//preprocessing(store_image_path+heart_phase+"/mask_manual"+"/"+folders_name[k], store_image_path+heart_phase+"/mask_manual"+"/"+folders_name[k]+"/border_zone/","bz");
...
for ( unsigned int k = 0; k < folders_name2.size() ;k++ ){
	...
	for ( unsigned int o = 0; o < patient_number[k] ;o++ ){
		...
		if (sgp1 ->check_black_image((mask_compute_myo+"final_myo_"+std::to_string(o)+".jpeg").c_str(),0,0)==true)
		{ 
			std::cout << "Delete black image: "<<"final_myo_"+std::to_string(o)+".jpeg"<<", "<<(mask_compute_scar+"final_scar_"+std::to_string(o)+".jpeg").c_str()<<std::endl;
			std::string commandwr2r;
			commandwr2r += "rm -rf ";
			commandwr2r += (mask_compute_myo+"final_myo_"+std::to_string(o)+".jpeg").c_str();
			int wrf2r= system(commandwr2r.c_str());

			std::string commandwr2r1;
			commandwr2r1 += "rm -rf ";
			commandwr2r1 += (mask_compute_scar+"final_scar_"+std::to_string(o)+".jpeg").c_str();
			int wrf2r1= system(commandwr2r1.c_str());
		
		}

		else{
			boost::filesystem::copy_file( (mask_compute_myo+"final_myo_"+std::to_string(o)+".jpeg").c_str(), (store_image_path+"image/"+folders_name2[k]+"/myo_"+std::to_string(co)+".jpeg").c_str(), boost::filesystem::copy_option::overwrite_if_exists);
			boost::filesystem::copy_file( (mask_compute_scar+"final_scar_"+std::to_string(o)+".jpeg").c_str(), (store_image_path+"image/"+folders_name2[k]+"/contour/scar_"+std::to_string(co)+".jpeg").c_str(), boost::filesystem::copy_option::overwrite_if_exists);
		co++;
		}

	}//end for
patient_number[k]=co;
cout << "new patient umber" <<co<<", "<< patient_number[k]<< std::endl;
}//end for
...
std::string mlp_expert_manual=manual_labels_path+"ED/"+"mask_manual/"+std::to_string(0)+"/";
for ( unsigned int k = 0; k < folders_name2.size() ;k++ )
{
	int up=0;
	int up2=0;
...
cout << "image preprocessing number:  " <<k<< std::endl;
	...
	if (label=="scar"){
		...
		if (sample_PB->GetMeasurementVector(0)[1]==80 && sample_PB->GetMeasurementVector(0)[2]==150 && sample_PB->GetMeasurementVector(0)[2]==230){low=0;up=1;}//10}
		...
		if (sgp->check_black_image(seg.c_str(),0,0)==false)
		{
			...
			if (low!=0&&up!=1){ BI->Image_binary((store_seg).c_str(),(store_seg).c_str(), low-2 ,up);}
	...
	if (label=="bz"){
		low=sample_PB->GetMeasurementVector(0)[2]-10;
		up=sample_PB->GetMeasurementVector(0)[3]-10;
...
//std:: vector<string> bzone =read_images_folder(mask_compute_border_zone1,1);

if (size_expert<2){
	for ( unsigned int x = 0; x < myo.size(); x++ ){
	boost::filesystem::copy_file( (mask_compute_myo1+myo[x]).c_str(), (mask_compute_myo+"final_myo_"+std::to_string(x)+".jpeg").c_str(), boost::filesystem::copy_option::overwrite_if_exists);
	boost::filesystem::copy_file( (mask_compute_scar1+scar[x]).c_str(), (mask_compute_scar+"final_scar_"+std::to_string(x)+".jpeg").c_str(), boost::filesystem::copy_option::overwrite_if_exists);

		}
//boost::filesystem::copy_file( mask_compute_border_zone1,mask_compute_border_zone,boost::filesystem::copy_option::overwrite_if_exists);
	}
...

{
	...
	//std:: vector<string> bzone1 =read_images_folder(mask_compute_border_zone1,0);
	...
	//std:: vector<string> bzone2 =read_images_folder(mask_compute_border_zone2,0);
	...
	// add images
		if (ex-1==0){
			for ( unsigned int x = 0; x < myo1.size(); x++ )
			{
				...
				//SM_norm->Normalize_image(bzone1[x].c_str(), bzone1[x].c_str(),normalization_threshold,1 );
				}//end for
		}//end if
		...
		for ( unsigned int x = 0; x < myo1.size(); x++ )
		{
			...
			B2->Image_binary_intensity_handle(myo2[x].c_str(),myo2[x].c_str(), threshold-2 ,255, value_frondground,0);
		...
		for ( unsigned int s = 0; s < scar1.size(); s++ )
		{
	...
	/*
		for ( unsigned int b = 0; b < bzone1.size(); b++ )
		{
			std::shared_ptr<denoising > SM_norm3 =std::make_shared<denoising>();
			SM_norm3->Normalize_image(bzone2[b].c_str(), bzone2[b].c_str(),normalization_threshold2,1 );
			
			std::shared_ptr<mergeimages> MI2b =std::make_shared<mergeimages>();
...
//std::shared_ptr< Segmentation_Pipeline > sgp3 =std::make_shared< Segmentation_Pipeline >();
	...
	if (sgp1->check_black_image((mask_compute_myo+myo[x]).c_str(),0,0)==false)
	{
		...
		atlas2->Image_segment_Thresholding((root+"kmeans_myo.jpeg").c_str(), (mask_compute_myo+"final_myo_"+std::to_string(x)+".jpeg").c_str(), threshold2, 255, false);
		std::shared_ptr<BinaryImage> BI2 =std::make_shared<BinaryImage>();
		BI2->Image_binary((mask_compute_myo+"final_myo_"+std::to_string(x)+".jpeg").c_str(),(mask_compute_myo+"final_myo_"+std::to_string(x)+".jpeg").c_str(), threshold2-2 ,255);
		for ( unsigned int i = 0 ; i < 1 ; ++i ){//2
								std::shared_ptr<denoising> SM1 =std::make_shared<denoising>(root);
								SM1->Median_smoothing((mask_compute_myo+"final_myo_"+std::to_string(x)+".jpeg").c_str(),(mask_compute_myo+"final_myo_"+std::to_string(x)+".jpeg").c_str(),1,1);
								}

	}
	else
	{
		boost::filesystem::copy_file( (mask_compute_myo+myo[x]).c_str(), (mask_compute_myo+"final_myo_"+std::to_string(x)+".jpeg").c_str(), boost::filesystem::copy_option::overwrite_if_exists);
	...
	if (sgp2->check_black_image((mask_compute_scar+scar[x]).c_str(),0,0)==false)
	{
		...
		atlas2->Image_segment_Thresholding((root+"kmeans_scar.jpeg").c_str(), (mask_compute_scar+"final_scar_"+std::to_string(x)+".jpeg").c_str(), threshold3, 255, false);
		std::shared_ptr<BinaryImage> BI =std::make_shared<BinaryImage>();
		BI->Image_binary((mask_compute_scar+"final_scar_"+std::to_string(x)+".jpeg").c_str(), (mask_compute_scar+"final_scar_"+std::to_string(x)+".jpeg").c_str(), threshold3 ,255);
		for ( unsigned int i = 0 ; i < 0 ; ++i ){//1, no smooth
								std::shared_ptr<denoising> SM1 =std::make_shared<denoising>(root);
								SM1->Median_smoothing((mask_compute_scar+"final_scar_"+std::to_string(x)+".jpeg").c_str(),(mask_compute_scar+"final_scar_"+std::to_string(x)+".jpeg").c_str(),1,1);
								}
	}
	else
	{
		boost::filesystem::copy_file( (mask_compute_scar+scar[x]).c_str(), (mask_compute_scar+"final_scar_"+std::to_string(x)+".jpeg").c_str(), boost::filesystem::copy_option::overwrite_if_exists);
...
/*
	if (sgp3->check_black_image((mask_compute_border_zone+border_zone[x]).c_str())==false)
	{
		atlas2->Image_segment_Thresholding((mask_compute_border_zone+border_zone[x]).c_str(), (mask_compute_border_zone+"final_myo_"+std::to_string(x)+".jpeg").c_str(), threshold, 255, false);
	}
	else
	{
		boost::filesystem::copy_file( (mask_compute_border_zone+border_zone[x]).c_str(), (mask_compute_border_zone+"final_myo_"+std::to_string(x)+".jpeg").c_str(), boost::filesystem::copy_option::overwrite_if_exists);
...
for ( unsigned int k = 0; k < folders_patient.size() ;k++ ){
	...
	std:: vector<string> mask_roi =read_images_folder((mask_compute_roi_new+folders_patient[k]+"/store_roi/" ).c_str(),0,false);		
