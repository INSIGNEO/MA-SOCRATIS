		...
		for ( unsigned int k = 0; k < patients.size(); k++ )
			{
				...
				for ( unsigned int o = n; o < n+nb_images[inverse_count]; o++ ){
					...
					myo_image_dl= myo_path+"myo"+save_number+".jpeg";
					epi_image_dl= myo_path+"epi"+save_number+".jpeg";
						...
						if (sgp->check_black_image(first_mask.c_str(),0,0)==false)
							{
								...
								for ( unsigned int i = 0 ; i < 40 ; ++i ){
									...
									BI2->Image_binary(first_mask.c_str(),first_mask.c_str(), low-2 ,255);
								...
								riob->remove_object(first_mask.c_str(),  first_mask.c_str() , 1, "None");
						...
						if (sgp->check_black_image(second_mask.c_str(),0,0)==false)
							{
								...
								for ( unsigned int i = 0 ; i < 40 ; ++i ){
									std::shared_ptr<BinaryImage> BI =std::make_shared<BinaryImage>();
									BI->Image_binary(second_mask.c_str(),second_mask.c_str(), low-2 ,255);
									std::shared_ptr<denoising> SM1 =std::make_shared<denoising>(root);
								...
								rbio->remove_object(second_mask.c_str(), second_mask.c_str() , 1, "None");
...
for ( unsigned int k = 0; k < patients.size(); k++ )
	{   
		...
		{	
			...
			if (classes==0)
			{	
				store_myo_path_mask=store_data_path+case_dl+"masks"+std::to_string((image_number))+".jpeg";
				std::string myo_path_image= myo_path+"myo"+save_number+".jpeg";
			...
			if (classes>0)
			{
				...
				store_myo_path_image_background=store_path_temporaly2+case_dl+"mask_"+std::to_string(o)+".jpeg";
				store_myo_path_image_forground=store_path_temporaly2+case_dl+"mask_"+std::to_string(half_number_of_images+o)+".jpeg";
			...
			{
				store_roi_path_image= store_data_path+case_dl+"test"+std::to_string(o)+".jpeg";
				roi_path=(store_roi_path+patients[inverse_count])+'/';
			...
			if (classes==-1)
			{
				store_roi_path_image= store_data_path+"/AtoB_"+patients[inverse_count]+"_"+std::to_string(check_number+1)+".jpeg";
				roi_path=(store_roi_path+'/'+patients[inverse_count])+'/';
				roi_path=roi_path+case_dl+"test"+save_number+".jpeg";
