...
for ( unsigned int k = 0; k < num_patient; k++ )
	{
		...
		for ( unsigned int kk = 0; kk < nb_images_patient; kk++ ){
			...
			predictm=pm.str();
			std::stringstream sm;
			sm << scar_path_patient <<std::to_string(kk)<< "/binary_input_image.jpg" ;
			myo=sm.str();
	...
	for ( unsigned int o = 0; o< patient_images.size(); o++){
		...
		if( image_file=="myo"){
				...
				if( fexists( path_of_code_results+patient_folders[k]+"/"+std::to_string(o)+"/comb_myo_b.jpg" )==true)
				{
					...
					boost::filesystem::copy_file( (path_of_code_results+patient_folders[k]+"/"+std::to_string(o)+"/comb_myo_b.jpg"), (predictpath_store+patient_folders[k]+"/"+image_file+"/myo_"+ std::to_string(o)+".jpg"), boost::filesystem::copy_option::overwrite_if_exists);
					up2=up2+1;
		...
		if( (image_file=="all"))
			{
				...
				{
					boost::filesystem::copy_file( (path_of_code_results+patient_folders[k]+"/"+std::to_string(o)+"/binary_combine_image.jpg"), (predictpath_store+patient_folders[k]+"/scar"+"/scar_"+ std::to_string(o)+".jpg"), boost::filesystem::copy_option::overwrite_if_exists);
					boost::filesystem::copy_file( (path_of_code_results+patient_folders[k]+"/"+std::to_string(o)+"/comb_myo_b.jpg"), (predictpath_store+patient_folders[k]+"/myo"+"/myo_"+ std::to_string(o)+".jpg"), boost::filesystem::copy_option::overwrite_if_exists);
					up2=up2+1;
...
//if(patient_folders[k]=="16"||patient_folders[k]=="17"||patient_folders[k]=="18"||patient_folders[k]=="19"||patient_folders[k]=="20"||patient_folders[k]=="2"||patient_folders[k]=="3"||patient_folders[k]=="4"||patient_folders[k]=="5"||patient_folders[k]=="6"||patient_folders[k]=="7"||patient_folders[k]=="8"||patient_folders[k]=="9")
...
//if((patient_folders[k]=="8")||(patient_folders[k]=="9"))	{	
	...
	if((store_label=="scar")||(store_label=="all"))	
	{
	for ( unsigned int o = 0; o< manual_new.size(); o++)
		{
		std::cout << "Patient number: "<<patient_folders[k]<< '\n';
